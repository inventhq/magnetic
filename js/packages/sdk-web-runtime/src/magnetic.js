...
(function(d) {
  ...
  function patch(el, n) {
    setAttrs(el, n);
    // Sync event bindings: add new, remove stale
    if (n.events) {
      for (var v in n.events) el.dataset["a_" + v] = n.events[v];
    }
    // Remove event attrs no longer present
    var da = el.dataset;
    for (var dk in da) {
      if (dk.indexOf("a_") === 0) {
        var ev = dk.slice(2);
    ...
    } else if (!n.children && n.text == null && n.tag != "input" && n.tag != "textarea") {
      // Node should be empty — clear any leftover children
      while (el.firstChild) {
        purgeKeys(el.firstChild);
