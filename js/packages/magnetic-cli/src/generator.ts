 ...
 */
export function generateBridge(scan: AppScan, config?: MagneticAppConfig, designJson?: string, contentInjection?: string): string {
  const lines: string[] = [];
  lines.push('// AUTO-GENERATED by @magnetic/cli — do not edit');
  lines.push(`import { createRouter } from '${scan.serverPkgPath}/router';`);

  // CSS framework imports (mode-dependent)
  if (designJson) {
    const designObj = JSON.parse(designJson);
    const cssMode: string = designObj.css || 'all';
    if (cssMode === 'all') {
      lines.push("import { generateAllCSS } from '@magneticjs/css';");
    } else if (cssMode === 'pages') {
      lines.push("import { extractCSS } from '@magneticjs/css';");
    } else if (cssMode === 'used') {
      lines.push("import { createExtractor } from '@magneticjs/css';");
    }
  }
  ...
  if (designJson) {
    ...
    if (cssMode === 'all') {
      ...
      // Mode "pages": render all static routes at init, collect union of classes,
      // extract only used utilities. SSE-safe (all navigable pages scanned).
      const staticRoutes = scan.pages
        .filter(p => !p.isCatchAll)
        .map(p => p.routePath.replace(/:([^/]+)/g, '0')); // replace :param with "0"
      lines.push(`var __routes = ${JSON.stringify(staticRoutes)};`);
      lines.push('');
      lines.push('function __collectClasses(node, out) {');
      lines.push('  if (node && node.attrs && node.attrs["class"]) {');
      lines.push('    var parts = node.attrs["class"].split(/\\s+/);');
      lines.push('    for (var i = 0; i < parts.length; i++) { if (parts[i]) out.add(parts[i]); }');
      lines.push('  }');
      lines.push('  if (node && node.children) {');
      lines.push('    for (var j = 0; j < node.children.length; j++) { __collectClasses(node.children[j], out); }');
      lines.push('  }');
      lines.push('}');
      lines.push('');
      lines.push('var __allClasses = new Set();');
      lines.push('for (var __ri = 0; __ri < __routes.length; __ri++) {');
      lines.push('  try { __collectClasses(render(__routes[__ri]), __allClasses); } catch(e) {}');
      lines.push('}');
      lines.push('var __cssCache = extractCSS(');
      lines.push('  { tag: "div", attrs: { "class": Array.from(__allClasses).join(" ") } },');
  ...
  // Content pipeline injection (build-time markdown → HTML)
  if (contentInjection) {
    lines.push('');
    lines.push(contentInjection);
  }
