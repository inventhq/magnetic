---
title: CSS & Styling
description: Design tokens, utility classes, custom CSS, and the CSS pipeline.
layout: docs
order: 4
---

# Magnetic CSS & Styling Reference

`@magneticjs/css` is a server-side utility CSS framework — zero client-side JS for styling. CSS is generated at build time from `design.json` tokens and injected as inline `<style>` in the SSR HTML.
